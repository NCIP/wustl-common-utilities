...
public class Graph<V, E> implements Serializable, Cloneable {
	...
	void validateVertex(V vertex) {
		...
		if (!containsVertex(vertex)) {
			throw new IllegalArgumentException(
					"specified vertex is not present in graph.");
		}
	}
	...
	private Set<List<V>> getVertexPaths(V source, V target,
			Set<V> verticesToIgnore) {
		...
		for (Map.Entry<V, E> entry : incomingEdgeMap.get(target).entrySet()) {
			...
			Set<List<V>> thePaths = getVertexPaths(source, srcSrc,
					verticesToIgnore);
