...
public class Tree<N, L> implements Serializable, Cloneable {
	...
	public <N1 extends N, L1 extends L> void addSubTree(N parent,
			Tree<N1, L1> subTree, L label, final boolean overWrite) {
		N1 subTreeRoot = subTree.getRoot();
		if (!overWrite && getChildren(parent).contains(subTreeRoot)) {
			return;
		}
		putChild(parent, subTreeRoot, label);
		for (Map.Entry<N1, L1> entry : subTree.getChildrenWithLabels(
				subTreeRoot).entrySet()) {
			addSubTree(subTreeRoot, subTree.subTree(entry.getKey()), entry
					.getValue(), overWrite);
		}
	}
